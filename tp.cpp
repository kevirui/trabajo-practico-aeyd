...
struct Proveedor { // Estructura para almacenar los proveedores de un componente
    int id;
    char nombre[101];
    float valor_unitario;
};

struct Componente { // Estructura para almacenar los componentes de un modelo
    int id;
...
struct ComponenteModelo { // Estructura para almacenar los componentes de un modelo
    int id_accesorio;
    int cantidad;
};

struct Modelo { // Estructura para almacenar los modelos de zapatillas
    int id;
    char descripcion[101];
    float precio_base;
    char temporada[10];
...
struct Pedido { // Estructura para almacenar los pedidos de zapatillas
    int id_pedido;
    int id_linea;
    int id_modelo;
    char fecha[11];
...
void inicializar_datos() { // Inicializa los datos de los componentes y modelos
    srand(time(NULL));
    for (int i = 0; i < num_componentes; i++) { // Inicializa los componentes
        ...
        for (int j = 0; j < 3; j++) { // Inicializa los proveedores de cada componente
    ...
    for (int i = 0; i < num_modelos; i++) { // Inicializa los modelos
        ...
        for (int j = 0; j < modelos[i].num_componentes; j++) { // Inicializa los componentes de cada modelo
...
void mostrar_componentes() { // Muestra la información de los componentes
    cout << "=== Información de Componentes ===" << endl; // Muestra la información de los componentes
    for (int i = 0; i < num_componentes; i++) { // Recorre los componentes
        cout << "\nID: " << componentes[i].id << endl; // Muestra el ID del componente
        ...
        for (int j = 0; j < componentes[i].num_proveedores; j++) { // Recorre los proveedores del componente
            cout << " - " << componentes[i].proveedores[j].nombre // Muestra el nombre del proveedor
                 << " ( Valor: $" << fixed << setprecision(2) << componentes[i].proveedores[j].valor_unitario << " )" << endl; // Muestra el precio del proveedor
        }
    }
}

void generar_pedidos(const char *archivo_pedidos) { // Genera pedidos de ejemplo
    ...
    for (int i = 0; i < 5; i++) { // Genera 5 pedidos de ejemplo
...
void procesar_pedidos(const char *archivo_entrada, const char *archivo_salida) { // Procesa los pedidos
    ...
    if (!archivo_in || !archivo_out) { // Si no se pudieron abrir los archivos
    ...
    while (fread(&pedido, sizeof(Pedido), 1, archivo_in) == 1) { // Lee los pedidos del archivo
        ...
        cout << "\n=== Pedido ID: " << pedido.id_pedido << " ===" << endl; // Muestra el ID del pedido
        ...
        cout << "Precio Base: $" << fixed << setprecision(2) << modelos[id_mod].precio_base << endl; // Muestra el precio base del modelo
        cout << "Temporada: " << modelos[id_mod].temporada << endl; // Muestra la temporada del modelo
        cout << "\nComponentes necesarios:" << endl; // Muestra los componentes necesarios

        for (int j = 0; j < modelos[id_mod].num_componentes; j++) { // Recorre los componentes del modelo
            ...
            cout << "\nID: " << componentes[id_comp].id + 1 << endl; // Muestra el ID del componente
            ...
            for (int p = 0; p < componentes[id_comp].num_proveedores; p++) { // Recorre los proveedores del componente
                cout << " - " << componentes[id_comp].proveedores[p].nombre // Muestra el nombre del proveedor
                     << " ( Valor: $" << fixed << setprecision(2) // Muestra el precio del proveedor
                     << componentes[id_comp].proveedores[p].valor_unitario << " )" << endl; // Muestra el precio del proveedor
                
                if (componentes[id_comp].proveedores[p].valor_unitario < menor_precio) { // Si el precio es menor al precio mínimo
            ...
            if (componentes[id_comp].stock < cantidad_necesaria) { // Si no hay stock suficiente
            ...
            cout << "Costo del componente: $" << fixed << setprecision(2) << costo_componente << endl; // Muestra el costo del componente
        }
        ...
        cout << "\nCosto total del pedido: $" << fixed << setprecision(2) << costo_pedido << endl; // Muestra el costo total del pedido
    }

    fclose(archivo_in); // Cierra el archivo de pedidos
    fclose(archivo_out); // Cierra el archivo de pedidos actualizados

    cout << "\n=== Resumen de pedidos actualizados ===" << endl; // Muestra un resumen de los pedidos actualizados
    archivo_out = fopen(archivo_salida, "rb"); // Abre el archivo de pedidos actualizados en modo lectura binaria
    while (fread(&pedido, sizeof(Pedido), 1, archivo_out) == 1) { // Lee los pedidos actualizados del archivo
        cout << "ID Pedido: " << pedido.id_pedido << ", ID Modelo: " << pedido.id_modelo + 1 // Muestra el ID del pedido y del modelo
             << ", Cantidad: " << pedido.cantidad << ", Costo: $" << fixed << setprecision(2) << pedido.costo << endl; // Muestra la cantidad y el costo del pedido
    }
    fclose(archivo_out); // Cierra el archivo de pedidos actualizados

    cout << "\nCosto total de todos los pedidos: $" << fixed << setprecision(2) << costo_total_pedidos << endl; // Muestra el costo total de los pedidos
}
